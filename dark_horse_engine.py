"""
Dark Horse Engine v14.4 (Final) – Full Scenario-Based Personalization
============================================================================
اصلاحات اعمال‌شده:
  + رفع باگ استخراج قطب مقابل در ارزش‌ها (سؤالات ۱ تا ۹)
  + کاهش آستانهٔ همسویی S و V از ۰.۶ به ۰.۵
  + استفاده از نام سبک فکری (trait_map) در highlights به‌جای شمارهٔ بُعد
  + افزودن فیلتر ایمنی M < 0.15 برای جلوگیری از نمایش رشته‌های فاقد جرقهٔ کافی
  + تغییر نام prestige_level به market_demand_level
  + مستندسازی حالت else در سناریوها با logger.warning
"""
...
class DarkHorseEngine:
    ...
    2: ["ساختاریافته", "حسی", "مشورتی", "آزمایشی", "آزمایش‌گر"],
    ...
    6: ["خلاق", "الگویاب", "تحلیلی", "مشورتی", "عمل‌گرا"],
    ...
    18: ["دستورپذیر", "خودمختار", "مشورتی", "تجربی", "موقعیتی"],
    ...
    22: ["ساخت‌وکار", "کتابخانه/آزمایشگاه", "طبیعت", "تعامل", "کامپیوتر"],
    ...
    def _extract_v_misaligned_poles(
        ...
        for v in value_choices:
            if not v or not v.strip() or not v.startswith('Q'):
                continue
            # استخراج صحیح شماره سؤال: Q1A → q_part='Q1', letter='A'
            letter = v[-1]
            q_part = v[:-1]
    ...
    def _generate_scenario_description(
        ...
        if m_aligned and s_aligned and v_aligned:
            ...
            logger.warning(
                f"Unexpected scenario for {major_name}: M={m_score:.2f}, S={s_score:.2f}, V={v_score:.2f}"
            )
            desc += (
                "خرده‌انگیزه‌های شما با این رشته همسو هستند. "
    ...
    def discover_individuality(
        ...
        for major_id, major_data in self.majors_db.items():
            try:
                ...
                # ✅ فیلتر ایمنی: اگر خرده‌انگیزه‌ها بسیار ضعیف باشند، رشته اصلاً نمایش داده نشود
                if m_score < 0.15:
                    continue
        ...
        return {
            ...
            "method": {
                ...
                "version": "14.4 (Fixed V-pole bug, adjusted thresholds, trait highlights, M safety filter)",
            },

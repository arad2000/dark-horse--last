"""
Dark Horse Engine v12.1 (Production-Ready, Bug-Fixed)
...
M Score (میکروموتیوها):
  ...
  (مقایسه بدون حساسیت به حروف بزرگ/کوچک)
...
class DarkHorseEngine:
    ...
    # ========================================================================
    # ۲. محاسبه M Score (فرمول ساده و شفاف - باگ حروف بزرگ/کوچک رفع شده)
    # ========================================================================
    def _compute_m_score(
        ...
        **مقایسه بدون حساسیت به حروف بزرگ/کوچک انجام می‌شود.**
        ...
        # باگ اصلی: اینجا باید همه کدها به lower() تبدیل شوند
        if isinstance(raw_codes, dict):
            major_set = {str(code).strip().lower() for code in raw_codes.keys()}
        elif isinstance(raw_codes, list):
            major_set = {str(code).strip().lower() for code in raw_codes}
        else:
            return 0.0, []
        ...
        # تبدیل انتخاب‌های کاربر به مجموعهٔ lower
        ...
        # جزئیات برای Evidence (با کد اصلی از فایل motives_map)
        matched_details = []
        for code in user_motives:
            code_lower = str(code).strip().lower()
            if code_lower in matched:
                # سعی می‌کنیم توضیح را با کلید اصلی (حروف بزرگ) پیدا کنیم

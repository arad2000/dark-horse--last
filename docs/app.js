// frontend/app.js — نسخهٔ نهایی V26.0
...
function saveSession() {
  ...
  try { localStorage.setItem('darkhorse_session', JSON.stringify(sessionData)); } catch (e) {}
}

function loadSession() {
  const saved = localStorage.getItem('darkhorse_session');
...
function clearSession() { localStorage.removeItem('darkhorse_session'); }
...
function renderSplash() {
  const hasSession = localStorage.getItem('darkhorse_session');
...
function analyzeValueStyle(answers) {
  const map = {
    ...
    'Q11A': { label: 'کار با داده‌ها', icon: '📊' }, 'Q11B': { label: 'کار با انسان‌ها', icon: '👥' },
    'Q12A': { label: 'خطرپذیری', icon: '🎲' }, 'Q12B': { label: 'ثبات‌طلبی', icon: '🛡️' },
    'Q13A': { label: 'نتیجهٔ فوری', icon: '⚡' }, 'Q13B': { label: 'اثر ماندگار', icon: '🏛️' },
    'Q14A': { label: 'استقلال کامل', icon: '🦅' }, 'Q14B': { label: 'تعلق سازمانی', icon: '🏢' },
    'Q15A': { label: 'تسلط بر یک حوزه', icon: '🎯' }, 'Q15B': { label: 'کنجکاوی بی‌پایان', icon: '🌌' }
  };
...
async function submitFeedback() {
  ...
  // ۱. ذخیره در localStorage (پشتیبان)
  try {
...
async function init() {
  ...
  if (localStorage.getItem('darkhorse_session')) {
    loadSession();
  }
  render();
}
